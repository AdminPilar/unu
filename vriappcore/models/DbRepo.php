...
class DbRepo extends PedrixAdo
{
    ...
    public function __construct()
    {
        ...
        //
        $this->setDB( "desarrollo_absmain" ); // modificar testing-produccion
    }
    ...
    function inGrado( $id )
    {
        ...
        $table = $this->db->get( "desarrollo_pilar3.docEstudios" ); // modificar testing-produccion
    ...
    //agregado unuv2.0
    function inDocenteNom( $id )
    {
        if( !$id ) return null;
        $row =$this->getSnapRow( "vwDocentes", "Id=$id");
        if( ! $row ) return null;
        return $row->DatosNom;
    }
